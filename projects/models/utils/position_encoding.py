...
class PositionEmbeddingSine(nn.Module):
    ...
    def _encode_xy(self, x, y):
        ...
        #this is added to make sure any floating difference, i.e. -.00012
        x = x.clamp(0.0, 1.0)
        y = y.clamp(0.0, 1.0) 
...
class PositionEmbeddingRandom(nn.Module):
    ...
    #this is implemented by Ahmad
    def encode_box_corners(self, boxes: torch.Tensor) -> torch.Tensor:
        boxes = boxes.to(torch.float)
        # [..., 2, 2] where [:,0]=(x0,y0), [:,1]=(x1,y1)
        corners = torch.stack([boxes[..., :2], boxes[..., 2:]], dim=-2).clamp(0.0, 1.0)
        pe_corners = self._pe_encoding(corners)   # [..., 2, C]
        return pe_corners.flatten(-2)             # [..., 2*C]
