...
class CoDETR(BaseDetector):
    ...
    def forward_train(self,
                      ...
                      gt_masks=None,
                      proposals=None,
                      gt_handside=None, # <--- This will now receive data
                      gt_interaction=None, # <--- This will now receive data
                      **kwargs):
        """
        Args:
        ...
        if self.with_query_head:
            detr_forward_results = self.query_head.forward_train(x, img_metas, gt_bboxes,
                                                          gt_labels, gt_bboxes_ignore,gt_handside=gt_handside,gt_interaction=gt_interaction) #### i added the side
    ...
    def simple_test_query_head(self, img, img_metas, proposals=None, rescale=False,return_hand_side=False):
        ...
        results_list, x = self.query_head.simple_test(
            x, img_metas, rescale=rescale, return_encoder_output=True,return_hand_side=return_hand_side)
        if return_hand_side:
            bbox_results = [
                bbox2result_hand_as_2_classes(det_bboxes,det_labels ,handsides, self.query_head.num_classes)
                for det_bboxes, det_labels,handsides in results_list
            ]
        else:
            bbox_results = [
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False,return_hand_side=False):
        """Test without augmentation."""
        ...
        return self.simple_test_query_head(img, img_metas, proposals, rescale,return_hand_side)
