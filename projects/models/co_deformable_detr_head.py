...
class CoDeformDETRHead(DETRHead):
    ...
    def get_bboxes(self,
        ...
        if hand_side_logits is not None:
            hand_side_logits = hand_side_logits[-1]
        result_list = []
        for img_id in range(len(img_metas)):
            cls_score = cls_scores[img_id]
            if hand_side_logits is not None:
                hand_side_logit = hand_side_logits[img_id]
            else:
                hand_side_logit=None
            ...
            proposals = self._get_bboxes_single(cls_score, bbox_pred,
                                                ...
                                                rescale, with_nms,hand_side_logit=hand_side_logit)
    ...
    def get_targets(self,
                    ...
                    gt_interaction_list=None):
        """Compute regression and classification targets for a batch of images.

        Args:
            cls_scores_list (list[Tensor]): List of [num_query, cls_out_channels].
            bbox_preds_list (list[Tensor]): List of [num_query, 4] normalized.
            gt_bboxes_list (list[Tensor]): List of [num_gts, 4].
            gt_labels_list (list[Tensor]): List of [num_gts].
            img_metas (list[dict]): List of meta info.
            gt_bboxes_ignore_list (list[Tensor], optional): List of ignored boxes.
            gt_handside_list (list[Tensor], optional): List of [num_gts] (optional handside labels).

        Returns:
            tuple: Either a 6-tuple or 7-tuple depending on handside availability:
                - labels_list
                - label_weights_list
                - bbox_targets_list
                - bbox_weights_list
                - num_total_pos
                - num_total_neg
                - gt_handside_targets_list (if available)
        """
        ...
        # Wrap in multi_apply with optional gt_handside_list
        if gt_handside_list is not None:
            outputs = multi_apply(
                self._get_target_single, cls_scores_list, bbox_preds_list,
                gt_bboxes_list, gt_labels_list, img_metas,
                gt_bboxes_ignore_list, gt_handside_list,gt_interaction_list)
            (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
            pos_inds_list, neg_inds_list, gt_handside_targets_list,gt_interaction_targets_list) = outputs
        else:
            outputs = multi_apply(
                self._get_target_single, cls_scores_list, bbox_preds_list,
                gt_bboxes_list, gt_labels_list, img_metas,
            ...
            (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
            pos_inds_list, neg_inds_list) = outputs
            gt_handside_targets_list = None
            gt_interaction_targets_list = None

        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))

        if gt_handside_targets_list is not None:
            return (labels_list, label_weights_list, bbox_targets_list,
                    bbox_weights_list, num_total_pos, num_total_neg,
                    gt_handside_targets_list,gt_interaction_targets_list)
        else:
            return (labels_list, label_weights_list, bbox_targets_list,
                    bbox_weights_list, num_total_pos, num_total_neg)
    ...
    def _get_target_single(self,
        ...
        Args:
            cls_score (Tensor): [num_query, cls_out_channels]
            bbox_pred (Tensor): [num_query, 4] in cx,cy,w,h normalized
            gt_bboxes (Tensor): [num_gts, 4] in x1,y1,x2,y2
            gt_labels (Tensor): [num_gts]
            img_meta (dict): Image metadata.
            gt_bboxes_ignore (Tensor, optional)
            gt_handside (Tensor, optional): [num_gts] handside labels (0=left, 1=right, 2=other)

        Returns:
            tuple[Tensor]: Either 6-tuple (default) or 7-tuple (with handside)
        """
        ...
        # ✅ Optional: return handside targets for matched GT
        if gt_handside is not None:
            # Will match same GTs as used for box assignment
            gt_handside_targets = gt_handside[sampling_result.pos_assigned_gt_inds]
            #import pdb
            #pdb.set_trace()
            #gt_handside_targets = gt_bboxes.new_full((num_bboxes,), self.num_classes, dtype=torch.long)
            #gt_handside_targets[pos_inds] = gt_handside[sampling_result.pos_assigned_gt_inds]
            
            
            gt_interaction_targets = gt_interaction[sampling_result.pos_assigned_gt_inds]
            
            return (labels, label_weights, bbox_targets, bbox_weights,
                    pos_inds, neg_inds, gt_handside_targets,gt_interaction_targets)
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            # Select matching hand-side logits
            if hand_side_logit is not None:
                hand_side_logit = hand_side_logit[bbox_index]
        else:
        ...
        if hand_side_logit is not None:
            hand_side_logit = hand_side_logit.argmax(dim=-1)
        ...
        if hand_side_logit is not None:
            hand_side_logit = hand_side_logit[valid_mask]
        ...
        if with_nms:
            ...
            if hand_side_logit is not None:
                hand_side_logit = hand_side_logit[keep_idxs][:cfg.max_per_img]
                return det_bboxes, det_labels,hand_side_logit
            else:
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False, return_encoder_output=False,return_hand_side=False):
        ...
        if return_hand_side:
            outs,hand_side_logits = self.forward(feats, img_metas,return_hand_side=return_hand_side)

            results_list = self.get_bboxes(*outs, img_metas, rescale=rescale, with_nms=with_nms,hand_side_logits=hand_side_logits)   
    ...
    def simple_test(self, feats, img_metas, rescale=False, return_encoder_output=False,return_hand_side=False):
        ...
        Returns:
            list[tuple[Tensor, Tensor]]: Each item in result_list is 2-tuple.
                ...
                with shape (n, ).
        """
        return self.simple_test_bboxes(feats, img_metas, rescale=rescale, return_encoder_output=return_encoder_output,return_hand_side=return_hand_side)