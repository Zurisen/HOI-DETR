...
def inference_detector(model, imgs,return_hand_side=False,return_interaction=False):
    ...
    # forward the model
    with torch.no_grad():
        results = model(return_loss=False, rescale=True,return_hand_side=return_hand_side, **data)
    if not is_batch:
        return results[0]
    else:
        return results
