dataset_type = 'Hands23CocoDataset'
data_root = '/lus/lfs1aip2/projects/u6ev/ahmad/datasets/hands23_data/'
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)
custom_imports = dict(
    imports=['mmdet.datasets.hands23_coco'], allow_failed_imports=False)
train_pipeline = [
    ...
    dict(
        type='Collect',
        keys=[
            'img', 'gt_bboxes', 'gt_labels', 'gt_handside', 'gt_interaction'
        ])
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
...
data = dict(
    ...
    workers_per_gpu=8,
    train=dict(
        type='Hands23CocoDataset',
        ann_file=
        '/lus/lfs1aip2/projects/u6ev/ahmad/datasets/hands23_data/annotations/train_h_first_second_full_corrected_auto_80_100.json',
        img_prefix=
        '/lus/lfs1aip2/projects/u6ev/ahmad/datasets/hands23_data/allMergedBlur/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadHandAnnotations', with_bbox=True),
            dict(type='RandomFlip', flip_ratio=0.5),
            ...
            dict(
                type='Collect',
                keys=[
                    'img', 'gt_bboxes', 'gt_labels', 'gt_handside',
                    'gt_interaction'
                ])
        ],
    ...
    val=dict(
        type='Hands23CocoDataset',
        ann_file=
        '/lus/lfs1aip2/projects/u6ev/ahmad/datasets/hands23_data/annotations/val_h_first_second_full_10.json',
        img_prefix=
        '/lus/lfs1aip2/projects/u6ev/ahmad/datasets/hands23_data/allMergedBlur/',
        pipeline=[
            dict(type='LoadImageFromFile'),
...
work_dir = 'work_dirs/hoi_swin_tiny'
auto_resume = False
gpu_ids = [0]
